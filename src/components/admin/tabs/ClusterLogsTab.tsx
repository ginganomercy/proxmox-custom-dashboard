        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200 bg-slate-50/50">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${logTab === 'tasks' ? 'bg-cyan-500 text-white shadow-md shadow-cyan-500/20' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-900'}`}
            >
              <Terminal className="w-3.5 h-3.5" /> Tasks
            </button>
            <button
              onClick={() => setLogTab('clusterlog')}
              className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${logTab === 'clusterlog' ? 'bg-cyan-500 text-white shadow-md shadow-cyan-500/20' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-900'}`}
            >
        ...
        {logTab === 'tasks' ? (
          <div className="p-2 overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-xs font-mono text-slate-700 divide-y divide-slate-100">
                {isLoadingTasks && clusterTasks.length === 0 ? (
                  ...
                  return (
                    <tr key={idx} className={`transition-colors ${isOK ? 'hover:bg-slate-50/80' : 'bg-red-50 hover:bg-red-100 text-red-700'}`}>
                      <td className="py-3 px-4 text-slate-500 whitespace-nowrap">{startStr}</td>
                      <td className="py-3 px-4 text-slate-500 whitespace-nowrap">{endStr}</td>
                      <td className="py-3 px-4 text-slate-600">{taskItem.node || targetNode}</td>
                      <td className="py-3 px-4 text-cyan-600 font-semibold">{taskItem.user || 'root@pam'}</td>
                      <td className="py-3 px-4 font-medium text-slate-800">{description}</td>
                      <td className="py-3 px-4">
                        {isOK ? (
                          <span className="text-emerald-500 font-bold">OK</span>
                        ) : (
                          <span className="text-red-600 font-bold">{statusText}</span>
                        )}
                      </td>
                    </tr>
                  );
                ...
                {isLoadingLogs && clusterLogs.length === 0 ? (
                  ...
                  return (
                    <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-4 text-slate-500">{logItem.node || targetNode}</td>
                      <td className="py-3 px-4 text-amber-600 font-medium">{logItem.tag || 'pvedaemon'}</td>
                      <td className="py-3 px-4 text-slate-400">{logItem.pid || 'N/A'}</td>
                      <td className="py-3 px-4 text-cyan-600 font-semibold">{logItem.user || 'root@pam'}</td>
                      <td className="py-3 px-4 text-slate-800 font-medium">{logItem.msg || logItem.type || 'cluster event'}</td>
                    </tr>
                  );
                })}
